...
NO_RICH_RESP_AMM = (
    "NO: Operazione consentita solo al richiedente, "
...
def rimuovi_rup(d_prat):
    ...
    if cs.EMAIL_RUP in d_prat:
        del d_prat[cs.EMAIL_RUP]
    if cs.INTERNO_RUP in d_prat:
        del d_prat[cs.INTERNO_RUP]
...
def pass_info(passcode, what=""):  # pylint: disable=R0911
    ...
    if what.startswith("te"):
        return passo[0]
    if what.startswith("fi"):
        return passo[1]
    if what.startswith("cm"):
        return passo[2]
    if what.startswith("al"):
        return passo[3]
    if what.startswith("ne"):
        return passo[4]
    if what.startswith("fu"):
        return passo[5]
...
Pratica:
    ...
    def check_next(self):
        ...
        _, doc, _, alleg, npasso, _ = cs.TABELLA_PASSI[cpasso]
...
def test_rdo_richiesta(d_prat: Pratica) -> str:
    ...
    return d_prat[cs.MOD_ACQUISTO] in (
        cs.TRATT_MEPA_40,
        cs.TRATT_UBUY_40,
        cs.TRATT_MEPA_143,
        cs.TRATT_UBUY_143,
...
def test_ordine_richiesto(d_prat: Pratica) -> str:
    ...
    return d_prat.get(cs.MOD_ACQUISTO, "") in (
        cs.INFER_5000,
        cs.TRATT_UBUY_40,
        cs.TRATT_UBUY_143,
    )
...
def auth_progetto_modificabile(d_prat: Pratica) -> str:  # pylint: disable=W0703,R0911
    ...
    if test_admin(d_prat.user) or test_richiedente(d_prat) or test_responsabile(d_prat):
        return YES
...
def send_email(eaddr, text, subj, attach=None):
    ...
    try:
        ft.send_email(
            CONFIG.config.get(cs.SMTP_HOST),
            sender,
            recipients,
            subj,
            text,
...
def check_access(new=False):
    ...
    if basedir:
        ...
        ft.set_pratica(d_prat[cs.NUMERO_PRATICA])
    else:
        d_prat = {
            cs.VERSIONE: cs.FILE_VERSION,
            cs.NUMERO_PRATICA: "-------",
...
def pratica_common(d_prat: Pratica):
    ...
    info["attach"] = [
        (a, _clean_name(a), a.startswith("A99"))
        for a in pdf_files
        if ALL_MATCH.match(a)
    ]
    info["allegati_mancanti"] = ", ".join(all_mancanti)
    info["stato_pratica"] = d_prat.get_passo("text")
...
def modifica_pratica(what):  # pylint: disable=R0912,R0915
    ...
    if what[0].lower() == "c":  # Chiudi pratica
        ...
        if err.startswith(NOT):
            fk.flash(err, category="error")
            ACQ.logger.warning(
                "Chiusura pratica rifiutata: %s. Utente %s, pratica %s",
                err,
                d_prat.user["userid"],
    ...
    if what[0].lower() == "a":  # Riapri pratica
        ...
        if err.startswith(NOT):
            fk.flash(err, category="error")
            ACQ.logger.warning(
                "Riapertura pratica rifiutata: %s. Utente %s, pratica %s",
                err,
                d_prat.user["userid"],
                d_prat[cs.NUMERO_PRATICA],
            )
...
def avvisi(d_prat: Pratica, _fase, _level=0):
    ...
    if not d_prat.get(cs.FIRMA_APPROV_RESP):
        fk.flash(
            "Occorre richiedere l'approvazione del responsabile dei fondi",
            category="info",
        )
...
def genera_documento(d_prat: Pratica, spec: list, filenum=0):
    ...
    ndata["rich_is_rup"] = d_prat.get(cs.EMAIL_RUP, "A") == d_prat.get(
        cs.EMAIL_RICHIEDENTE, "B"
    )
    ndata[cs.SEDE] = CONFIG.config[cs.SEDE]
    ndata[cs.RICH_IS_RESP] = (
        d_prat[cs.EMAIL_RICHIEDENTE] == d_prat[cs.EMAIL_RESPONSABILE]
    )
...
def costo_voce(voce):
    ...
    iva = importo * (int(voce["iva"]) / 100.0) if voce["iva"] else 0.0
    return importo, iva
...
def _voce(voce):
    ...
    v_ivav = round(v_imp * v_ivap / 100.0, 2)
    v_tot = v_imp + v_ivav
    return [
        s_desc,
        f"{v_imp:.2f}",
...
def login():
    ...
    form = fms.MyLoginForm(
        cs.DATADIR,
        uid,
        pwd,
        CONFIG.config.get(cs.LDAP_HOST),
        CONFIG.config.get(cs.LDAP_PORT),
        formdata=fk.request.form,
    )
...
def about():  # pylint: disable=R0915
    ...
    html.append(
        fmt.format("latex.py", latex.__version__, latex.__date__, latex.__author__)
    )
    html.append(
        fmt.format("table.py", ft.tb.__version__, ft.tb.__date__, ft.tb.__author__)
    )
    html.append(
        fmt.format(
            "Flask",
    ...
    if test_admin(user):
        ...
        html.append("<tr><td><table cellpadding=3 border=1>")
        config_spec = (
            f"{cs.CONFIG_FILE}, letto: {time.asctime(time.localtime(CONFIG.time))}"
        )
        html.append(
            f"<tr><th colspan=2> File di configurazione ({config_spec})  </th></tr>"
        )
        for key in cks:
...
def procedi():
    ...
    if not (d_prat := check_access()):
        return fk.redirect(fk.url_for("start"))
    if status_not_ok(
        d_prat, (CdP.GEN, CdP.AUD, CdP.DCI, CdP.ORD, CdP.ALS, CdP.PRO, CdP.OGP, CdP.ROG)
    ):
        fk.flash(ILLEGAL_OP, category="error")
        return pratica_common(d_prat)
    ret = d_prat.next()
...
def modificaprogetto():  # pylint: disable=R0912,R0915,R0911,R0914
    ...
    if mod_acquisto := fk.request.form.get(cs.MOD_ACQUISTO):
        ...
        if len(d_prat[cs.TAB_PASSI]) <= 1:
            d_prat[cs.TAB_PASSI] = (
                [CdP.GEN] if mod_acquisto == cs.GENERIC else [CdP.INI]
            )
    ...
    if prog.validate():
        ...
        d_prat[cs.NOME_RESPONSABILE] = nome_da_email(
            d_prat[cs.EMAIL_RESPONSABILE], True
        )
...
def inviaprogetto():
    ...
    if cannot_go:
        ...
        return pratica_common(d_prat)
    testo = cs.TESTO_APPROVAZIONE.format(
    ...
    if ret:
        fk.flash(
            ...
            f"{d_prat.get(cs.EMAIL_RESPONSABILE)}",
            category="info",
        )
        d_prat.next()
...
def inviaproposta():  # pylint: disable=R0914
    ...
    testo = cs.TESTO_INVIA_PROPOSTA.format(**d_prat) + cs.DETTAGLIO_PRATICA.format(
        **d_prat
    )
...
def inviadecisione():  # pylint: disable=R0914
    ...
    testo = cs.TESTO_INVIA_DECISIONE.format(**d_prat) + cs.DETTAGLIO_PRATICA.format(
        **d_prat
    )
    ...
    genera_documento(
        d_prat, (cs.DOC_DECISIONE, doc_opts)
    )  # Elimina barra "provvisorio"
    return pratica_common(d_prat)
...
def approvaprogetto():
    ...
    salvapratica(d_prat)
    subj = (
        "Notifica approvazione progetto di acquisto. Pratica: "
        + d_prat[cs.NUMERO_PRATICA]
    )
...
def rich_autorizzazione():
    ...
    if cannot_go:
        ...
        return pratica_common(d_prat)
    subj = (
        "Richiesta autorizzazione progetto di acquisto. Pratica: "
        + d_prat[cs.NUMERO_PRATICA]
    )
...
def modificardo():
    ...
    if fk.request.method == "POST":
        ...
        rdo = fms.RdO(**d_prat)
    ddp = {
        "title": "Immissione dati per RDO su MePA",
...
def modificaordine():  # pylint: disable=R0914
    ...
    ddp = {
        "title": "Immissione dati per ordine",
        "subtitle": f"Pratica N. {d_prat['numero_pratica']}",
...
def modificaproposta():  # pylint: disable=R0914
    ...
    ddp = {
        "title": "Immissione dati per proposta di aggiudicazione",
        "subtitle": f"Pratica N. {d_prat['numero_pratica']}",
...
def modificadecisione():  # pylint: disable=R0914
    ...
    if fk.request.method == "POST":
        if decis.validate(d_prat):
            ...
            storia(
                d_prat,
                what + " decisione a contrarre N. " f"{d_prat[cs.NUMERO_DECISIONE]}",
            )
            ndecis = int(d_prat[cs.NUMERO_DECISIONE].split("/")[0])
            if ft.check_dupl_decis(ndecis):
                fk.flash(
                    "Il numero di decisione "
    ...
    ddp = {
        "title": "Immissione dati per decisione di contrarre",
        "subtitle": f"Pratica N. {d_prat['numero_pratica']}",
...
def upload():  # pylint: disable=R0914,R0911,R0912
    ...
    if tipo_allegato == cs.ALL_GENERICO and os.path.exists(fpath):
        ...
        fk.flash(
            "Devi modificare il nome del file (o rimuovere quello esistente)",
            category="error",
        )
        return pratica_common(d_prat)
...
def lista_pratiche(filtro, anno, ascendente):  # pylint: disable=R0915,R0912,R0914
    ...
    ACQ.logger.info(
        "URL: /pratiche/%s/%s/%s (%s)", filtro, anno, ascendente, fk.request.method
    )
    user = user_info()
    fk.session[cs.BASEDIR] = ""
    anno = int(anno)
    ...
    return fk.render_template(
        "lista_pratiche_per_anno.html",
        filtro=filtro,
        test_mode=CONFIG.config.get(cs.TEST_MODE),
        sede=CONFIG.config[cs.SEDE],
        years=years,
        year=theyear,
        dlist=doclist.records,
        title=title,
    )
...
def trovapratica():  # pylint: disable=R0912,R0914,R0915
    ...
    if fk.request.method == "POST":
        ...
        ACQ.logger.debug(
            "trova - anno: %d, rich. %s, resp: %s, rup: %s, descriz: %s",
            anno,
            match_rich,
            match_resp,
            match_rup,
        ...
        try:
            lista, expr = ft.trova_pratiche_2(
                anno,
                stato,
                match_rich,
                match_resp,
...
def invia_obblig():
    ...
    if d_prat[cs.MOD_ACQUISTO] in (
        ...
        spec = f" {d_prat.get(cs.NUMERO_NEGOZIAZIONE)}"
    else:
        spec = ""
    params = {
        "fornitore": d_prat[cs.FORNITORE_NOME],
        "id": spec,
    ...
    if ret:
        ...
        fk.flash(
            "Errore invio di obbligazione giuridicamente perfezionata "
            f"per la pratica {d_prat[cs.NUMERO_PRATICA]} a: {email}",
            category="error",
        )
    return pratica_common(d_prat)
...
def housekeeping():
    ...
    fk.session.clear()
    return fk.render_template(
        "noaccess.html", test_mode=CONFIG.config.get(cs.TEST_MODE)
    )
...
def sortcodf(field):
    ...
    ncodf = ft.FTable(
        (cs.DATADIR, "codf.json"), sortable=("Codice", "email_Responsabile")
    )
...
def addcodf():
    ...
    return ncodf.render_item_as_form(
        "Aggiungi Codice Fu.Ob.", cfr, fk.url_for("addcodf"), errors=cfr.errlist
    )
...
def download(_unused):
    "Download"
    return fk.render_template(
        "tbd.html", test_mode=CONFIG.config.get(cs.TEST_MODE), goto="/"
    )
...
def adduser():
    ...
    if test_admin(user):
        ...
        if fk.request.method == "POST":
            ...
            ACQ.logger.debug("Validation errors: %s", cfr.errlist)
        return users.render_item_as_form(
            "Aggiungi utente",
            cfr,
            fk.url_for("adduser"),
            errors=cfr.errlist,
