...
class SplittedAttachment:
    ...
    def __init__(self, destdir, pdffile, debug=False):
        ...
        self.pagefiles = [
            os.path.join(destdir, f"atc_page-{np}.pdf") for np in range(npages)
        ]
...
def do_attachments(fname, atcs, debug=False):
    ...
    with open(sfname, "r", encoding="utf8") as src, open(
        fname, "w", encoding="utf8"
    ) as dst:
        while True:
            line = src.readline()
            if not line:
...
def makepdf(  # pylint: disable=R0912,R0913,R0914,R0915,R0917
    destdir,
    pdfname,
    template,
    remove=False,
